...
class PodcastTranscriber {
    ...
    async pollTranscriptions() {
        try {
            ...
            if (result.success) {
                this.updateTranscriptions(result.transcriptions, result.current_sentence);
            }
        } catch (error) {
    ...
    updateTranscriptions(transcriptions, currentSentence) {
        ...
        if (JSON.stringify(transcriptions) !== JSON.stringify(this.transcriptions)) {
    ...
    renderTranscriptions() {
        ...
        this.transcriptions.forEach(transcription => {
            ...
            sentenceDiv.innerHTML = this.formatSentenceWithClickableWords(transcription.text);
    ...
    renderCurrentSentence() {
        ...
        if (this.currentSentence) {
            ...
            sentenceDiv.innerHTML = this.formatSentenceWithClickableWords(this.currentSentence);
            container.appendChild(sentenceDiv);
    ...
    displayWordDefinition(word, definition) {
        ...
        if (definition.similar_words) {
            ...
            if (english.length > 0 || german.length > 0) {
                ...
                if (english.length > 0) {
                    html += `<div class="similar-words-content">Similar words in English: ${english.join(', ')}</div>`;
                }
                
                if (german.length > 0) {
                    html += `<div class="similar-words-content">Similar words in Deutsch: ${german.join(', ')}</div>`;
                }
    ...
    updateUI() {
        if (this.isRecording) {
            ...
            this.statusIndicator.querySelector('.status-dot').style.backgroundColor = '#FF4444';
        } else {
            this.playButton.classList.remove('recording');
            this.playIcon.textContent = '▶';
            this.statusText.textContent = 'Ready to listen';
            this.statusIndicator.querySelector('.status-dot').style.backgroundColor = '#4CAF50';
        }
    }
